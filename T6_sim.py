...
def time_error(circuit, qutrits, t):
    ...
    p = np.exp(-t / T0)
    dpg_t = QutritDepolarizingChannel((1.000000000000001 - p) * 3 / 4)
    for q in qutrits:
...
def graph(line, line2, c,s,t):
    ...
    ax.scatter(t, line, color='g', s=5, label='теор')
    ax.scatter(t, line2, color='black', s=5, label='теор2')
    print(line)
    print(line2)
    print(c)
    print(s)
...
#main(T, 20, N)
c, s, t = main(T, 5, nn)
line = 1 - (1-np.exp(-t/25))/2
Q = 1 - np.exp(-t / 25)
line2 = 1 - (10 * (1-Q)**3 * Q**2 + 10 * (Q)**3 * (1-Q)**2 + 5 * (1-Q)**1 * Q**4)
graph(line, line2,c,s,t)
...
ax.set_ylabel('ms2')
ax.set_zlabel('fid')
plt.title('P1 = 0.99, dipolarize, (P1 = 0.999 curve)')
plt.legend()
plt.grid()
plt.show()
