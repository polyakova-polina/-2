...
def encoding(cirquit, q_mask, v, vb):
    for o in range(B):
        ...
        u11 = U(R(0, v[0 + o * 4], 1, 2), 'Rx(-π)12')
        u21 = U(R(0, v[1 + o * 4], 1, 2), 'Rx(-π)12')
        u12 = U(R(0, v[2 + o * 4], 0, 2), 'Rx(-π)12')
        u22 = U(R(0, v[3 + o * 4], 0, 2), 'Rx(-π)12')
        ...
        u11r = U(R(0, -v[0 + o * 4], 1, 2), 'Rx(-π)12')
        u21r = U(R(0, -v[1 + o * 4], 1, 2), 'Rx(-π)12')
        u12r = U(R(0, - v[2 + o * 4], 0, 2), 'Rx(-π)12')
        u22r = U(R(0, - v[3 + o * 4], 0, 2), 'Rx(-π)12')
...
def operation(v):
    ...
    for i1 in range(2):
        for i2 in range(2):
            for i3 in range(2):
                ...
                encoding(circuit1, Q_mask, v, [1.570802692970867, 1.5707661293897417, -1.5708572752525478, 1.5707962052177018, -0.5168984632045052, 6.656076801648407e-06, -1.4316450374797591, 3.141592653589793]
)
    ...
    for i in range(len(v)):
        dobavka += min(abs(v[i] - np.pi), abs(v[i])) / len(v)

    return ans / 8 + dobavka / norm
 #   return ans / 8
...
for i in range(24):
    bnds.append((0, np.pi))
#guess = [np.pi/2, np.pi/2, 0, 0] + [0, np.pi/2, 0, 0] + [np.pi/2, 0, 0, 0] + [np.pi/2, 0, 0, 0] + [0, np.pi/2, 0, 0] + [np.pi/2, 0, 0, 0]
#guess = np.array([np.pi, np.pi, 0, 0,    0, 0, 0, 0,    np.pi, 0, 0, np.pi,     0, 0, 0,0,     np.pi, np.pi, 0, 0,      0, 0, 0, 0])
#print('oooooooooo', operation(guess))
options = {'maxiter': 1000}
norm = np.pi * 500000000
