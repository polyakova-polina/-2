...
def error(circuit, qutrits, ind):
    ...
    if rv < p * 10000:
        ...
        for i in range(n):
            ...
            mz = R(0, np.pi, 0, 1) @ R(np.pi / 2, np.pi, 0, 1)
            mi = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
            mat = (a1/a)*mx + (a2/a)*my + (a3/a)*mz + (a4/a)*mi
            matrix = U(mat)
            circuit.append(matrix(qutrits[i]), strategy=InsertStrategy.INLINE)
...
mat = p * mi + (a2 * mx + a3 * my + a4 * mz) / sss
er_gate = U(mat)
print('коэф',p, a2 / sss, a3 / sss, a4 / sss)
print('сумма', p + (a2 + a3 +a4) / sss)
circuit1.append(er_gate(qutrits1[0]), strategy=InsertStrategy.INLINE)

circuit1.append(er_gate(qutrits1[1]), strategy=InsertStrategy.INLINE)
